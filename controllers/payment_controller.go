...
var paymentService = service.PaymentService(repository.NewPayRepo())
...
func UpdatePayment(rw http.ResponseWriter, r *http.Request) {
	...
	json.NewEncoder(rw).Encode("Payment is updated by PUT method !")
	json.NewEncoder(rw).Encode(Payment)
}

func UpdatePa(rw http.ResponseWriter, r *http.Request) {
	rw.Header().Set("Content-Type", "application/json")
	var Payment models.Payment
	json.NewDecoder(r.Body).Decode(&Payment)
	paymentService.UpdatePa(Payment)
	json.NewEncoder(rw).Encode("Payment is updated by PATCH method !")
	json.NewEncoder(rw).Encode(Payment)
}
